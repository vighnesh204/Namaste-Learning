...
const Home = () => {
    ...
    // Fetch live courses data dynamically
    ...
    useEffect(() => {
        const fetchCourses = async () => {
            try {
                ...
                if (res?.length > 0) {
                    // Fallback Logic/Popular Picks: Use the first batch of courses
                    setPopularCourses(res.slice(0, 5));
                    
                    // Top Enrollments: Sort courses by their student enrollment arrays
                    const sortedByEnrollments = [...res].sort((a, b) => (b.studentsEnrolled?.length || 0) - (a.studentsEnrolled?.length || 0));
                    setTrendingCourses(sortedByEnrollments.slice(0, 5));
                }
            } catch (error) {
                console.error("Failed to retrieve courses for the homepage", error);
            }
        }
        fetchCourses();
    }, [])
